import type {
  NumeralInformation,
  StatementInformation,
} from "@interfaces/index";

export const data_numeral_11_9: NumeralInformation[] = [
  {
    id: 1,
    lawDescription: "Listado de Docentes UNT - Pregrado.",
    lawSubInformation:
      "Transparencia de Universidades – Ley 30220 - Artículo 11 - Numeral 11.9",
    buttonTooltip: "Ir a la Página de Docentes UNT - Pregrado",
    filePath: "/numeral-11.9/docentes-pregrado",
    icon: "fa-solid fa-chalkboard-user",
    buttonDescription: "Enlace",
    alternButtonIcon: "fa-solid fa-arrow-up-right-from-square",
  },
  {
    id: 2,
    lawDescription: "Listado de Docentes UNT - Posgrado.",
    lawSubInformation:
      "Transparencia de Universidades – Ley 30220 - Artículo 11 - Numeral 11.9",
    buttonTooltip: "Ir a la Página de Docentes UNT - Posgrado",
    filePath: "/numeral-11.9/docentes-posgrado",
    icon: "fa-solid fa-chalkboard-user",
    buttonDescription: "Enlace",
    alternButtonIcon: "fa-solid fa-arrow-up-right-from-square",
  },
];

export const data_11_9_pregrado: StatementInformation[] = [];

export const data_11_9_posgrado: StatementInformation[] = [
  {
    id: 1,
    lawDescription: "Listado de Docentes UNT - Posgrado - Año 2019",
    lawSubInformation:
      "Transparencia de Universidades – Ley 30220 - Artículo 11 - Numeral 11.9",
    icon: "fa-solid fa-chalkboard-user",
    rangeStatementItem: [
      {
        buttonDescription: "2019 - I",
        buttonTooltip:
          "Descargar Listado de Docentes UNT - Posgrado - Año 2019 I",
        filePath:
          "/files/numeral-11.9/posgrado/2019/docentes_posgrado_2019_I.pdf",
      },
      {
        buttonDescription: "2019 - II",
        buttonTooltip:
          "Descargar Listado de Docentes UNT - Posgrado - Año 2019 II",
        filePath:
          "/files/numeral-11.9/posgrado/2019/docentes_posgrado_2019_II.pdf",
      },
    ],
  },
  {
    id: 2,
    lawDescription: "Listado de Docentes UNT - Posgrado - Año 2020",
    lawSubInformation:
      "Transparencia de Universidades – Ley 30220 - Artículo 11 - Numeral 11.9",
    icon: "fa-solid fa-chalkboard-user",
    rangeStatementItem: [
      {
        buttonDescription: "2020 - I",
        buttonTooltip:
          "Descargar Listado de Docentes UNT - Posgrado - Año 2019 I",
        filePath:
          "/files/numeral-11.9/posgrado/2020/docentes_posgrado_2020_I.pdf",
      },
      {
        buttonDescription: "2020 - II",
        buttonTooltip:
          "Descargar Listado de Docentes UNT - Posgrado - Año 2019 II",
        filePath:
          "/files/numeral-11.9/posgrado/2020/docentes_posgrado_2020_II.pdf",
      },
    ],
  },
  {
    id: 3,
    lawDescription: "Listado de Docentes UNT - Posgrado - Año 2021",
    lawSubInformation:
      "Transparencia de Universidades – Ley 30220 - Artículo 11 - Numeral 11.9",
    icon: "fa-solid fa-chalkboard-user",
    rangeStatementItem: [
      {
        buttonDescription: "2021 - I",
        buttonTooltip:
          "Descargar Listado de Docentes UNT - Posgrado - Año 2021 I",
        filePath:
          "/files/numeral-11.9/posgrado/2021/docentes_posgrado_2021_I.pdf",
      },
    ],
  },
];
